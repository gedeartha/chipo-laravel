            ...
            <div class="grid grid-cols-12 gap-10">
                <div class="col-span-8">
                    <div class="shadow-md bg-white rounded-xl p-5">
                        <div class="font-extrabold text-3xl text-primary mb-4">Edit Topping</div>
                        <hr class="mb-4" />
                        ...
                        <form action="{{ $topping->id }}/update" method="POST" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf

                            <!-- Name -->
                            <div class="mb-3">
                                <x-label for="name" value="Nama Topping" />
                                <x-input id="name" class="block mt-2 w-full" type="text" name="name"
                                    placeholder="Nama topping" value="{{ old('name', $topping->name) }}" required
                                    autofocus />
                            ...
                            <div class="mb-3">
                                ...
                                <textarea rows="4" id="description" class="block mt-2 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                    type="text" name="description" placeholder="Masukkan deskripsi" required
                                    autofocus>{{ old('name', $topping->description) }}</textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <x-input id="price" class="block mt-2 w-full" type="number" name="price"
                                    placeholder="Masukkan harga" value="{{ old('price', $topping->price) }}" required
                                    autofocus />
                            ...
                            <div class="mb-3">
                                ...
                                <select id="status" name="status"
                                    ...
                                    <option value="Tersedia" @if ($topping->status == 'Tersedia') selected @endif>
                                        Tersedia</option>
                                    <option value="Tidak Tersedia" @if ($topping->status == 'Tidak Tersedia') selected @endif>
                                        Tidak
                                        Tersedia
                                    </option>
                                </select>
                ...
                <div class="col-span-4">
                    <div class="block">
                        <img class="h-full w-full rounded-2xl" src="{{ Storage::url('upload/') . $topping->image }}"
                            alt="Telur Puyuh" />
                    </div>
                    <div class="flex justify-between mt-4 mb-2 space-x-2">
                        <div class="font-extrabold text-3xl text-primary">{{ $topping->name }}</div>
                    ...
                    <div class="-mt-2 text-md text-gray-500">{{ $topping->description }}</div>

                    <div class="mt-4">
                        <form action="{{ $topping->id }}/delete" method="POST">
                            @csrf
                            @method('delete')
                            <x-button-delete>
                                Hapus
